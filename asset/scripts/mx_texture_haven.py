...
# 创建主窗口
class MX_TextureHaven(QWidget):
    def __init__(self,asset_path):

        super(MX_TextureHaven, self).__init__()
        # 获取Maya主窗口的指针
        ...
        for widget in self.mayaMainWindow.children():
            ...
            if widget.objectName() == "textureHavenUI":
                widget.close()
                widget.deleteLater()
                break
                
                       
        self.setObjectName("textureHavenUI")                      
        self.setWindowTitle("texture haven")
        ...
        thumbs = os.listdir(os.path.join(self.asset_path,"tex_thumbs"))
        ...
        if(len(thumbs))>0:            
            ...
            for thumb in thumbs:
                self.image_paths.append(os.path.join(self.asset_path,"tex_thumbs",thumb))
    ...
    def add_to_scene(self,asset_id):
        ...
        if(len(sel)>0):
            ...
            response = urllib.request.urlopen(requests) #get texture info
            ...
            new_shader = cmds.shadingNode( 'aiStandardSurface', asShader=True, n=asset_id + "_shader")
            ...
            cmds.warning("you have to select at least one geometry!")
    ...
    def update(self):
        ...
        for key in content.keys():
            ...
            thumb_path = os.path.join(self.asset_path,"tex_thumbs/{}.png".format(key))
        ...
        new_win = MX_TextureHaven(self.asset_path) 
